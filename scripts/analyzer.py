...
- 不同板块的传导链示例（必须达到这个具体程度）：
  能源：原油↑→石脑油↑→乙烯/丙烯↑→PVC/PP塑料粒子↑→包装/建材终端品↑
  化工：天然气↑→合成氨↑→尿素/磷酸二钵↑→小麦/玉米种植成本↑→粮食价格↑
  黄金：地缘冲突↑→中央银行增持黄金储备↑→实物黄金ETF资金流入↑→紫金矿业/山东黄金股价↑→黄金首饰零售价↑
  军工：冲突升级↑→各国国防预算↑→导弹/无人机采购订单↑→碳纤维/钙合金原材料需求↑→光威复合材料/中简科技营收↑
...
3. 每条传导链是否有4级以上（至少4个→）？
4. 每个节点是否用了具体产品名（如"乙烯""尿素""碳纤维"）而非泛称（如"化工原料""下游产品"）？有泛称必须替换！
5. 每条链是否标注了具体涨跌幅数据（如+25%、+120美元/吨）？
6. 总输出是否超过3000字？"""
